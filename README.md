...
For persistent, subscription-backed Codex chat, use the pinned app-server version and an exact model selector:

```sh
codex --version   # must report codex-cli 0.153.4
codex login
cargo run --release -- --model codex:gpt-5.6-sol
```

App-server-backed Codex requires a safe, owner-only regular authentication file at `$CODEX_HOME/auth.json` (normally `~/.codex/auth.json`).
...
If [Claude Code](https://docs.anthropic.com/en/docs/claude-code) or [Codex](https://github.com/openai/codex) is installed and signed in, subscription-backed choices appear in the picker (Claude Pro/Max, ChatGPT Plus/Pro) instead of using a metered API key. `claude-code` and bare `codex` mean **use that CLI's configured default**; bare `codex` stays stateless and starts one process per turn. Explicit selectors pass a model override to the CLI: for example, `/model claude-code:sonnet` uses Claude's latest Sonnet alias, while `/model codex:gpt-5.6-sol` pins that Codex model and enables persistent app-server reuse for ordinary solo turns. Claude aliases come from the installed CLI's documented interface; Codex shows the current list-visible upstream models from its bundled catalog without executing a CLI during discovery. You can enter any provider-qualified full model ID directly with `/model`, even when it is not listed—the CLI remains authoritative about account access.
...
We never read or store a subscription token — each CLI owns its authentication. Shaltaiboltai renders the event stream from fresh Claude/bare-Codex children or from an owned `codex app-server --stdio` process for exact Codex models. The child drives its own tool loop, while Shaltaiboltai maps the active permission snapshot onto every launch instead of inheriting ambient authority:

| Permission | Codex CLI | Claude Code CLI |
|---|---|---|
| Read Only | attested workspace-read profile; exact models use app-server | `plan` + safe mode, read/search tools only |
...
Headless children cannot route an inner approval prompt back into this TUI, so their inner approval policy is fail-closed. Codex Read Only uses a default-deny permission profile with minimal runtime reads, the captured workspace roots as read-only, and network disabled; legacy `--sandbox read-only` is never used for this path. Before an app-server launch, Shaltaiboltai creates a private process-scoped `CODEX_HOME`, links only a safe regular `auth.json` from the signed-in home, and explicitly denies model tools both authentication locations. The isolated home removes ordinary user configuration and trust state; pinned session overrides prevent ambient user or ordinarily untrusted project configuration, global Codex instructions, skills, MCP/browser surfaces, shell snapshots, secret-bearing environment inheritance, and nested Codex agents from widening the child. Managed/system configuration can still be discovered, so the effective boundary and every repository instruction source reported by app-server are independently attested. Repository `AGENTS.md` instructions inside the reviewed workspace may still apply. The app-server must report Codex CLI **0.153.4**, the exact model/provider at both response and thread level, canonical workspace roots, no model fallback, `never` approvals, the expected network boundary, and the fresh permission-profile ID when applicable during `initialize` + `thread/start`; only then does Shaltaiboltai send the first model-bearing `turn/start`. An attestation mismatch fails before that turn and never silently falls back to `codex exec`.

All app-server-backed Codex use—ordinary solo turns with an exact `codex:<model>` selector and team/read-only turns—currently requires exactly CLI 0.153.4 and a safe file-backed login at `$CODEX_HOME/auth.json`; keychain-only login state and unreviewed protocol versions fail closed. Bare `codex` remains available through the stateless `codex exec` path. Claude Code cannot express the protected-path carve-outs required by the normal workspace policy, so constrained Claude turns are advisory-only; use the Anthropic API provider for app-owned, policy-mediated edits. Claude Code editing requires the explicit Full Access preset. Full Access is available only through the startup policy or the two-step `/permissions` confirmation; there are no provider-specific bypass switches. An unavailable model is reported as an error and is never silently replaced.

Clean ordinary solo turns with the same exact `codex:<model>`, saved-session identity, instructions, history, and authority reuse one owned app-server process and native thread while this TUI is running. A model, session, instruction, or history change, an image handoff, cancellation, an authority change, or uncertain delivery retires that process before a fresh attested thread is created. Native Codex state remains ephemeral: restarting Shaltaiboltai creates a fresh thread and hands off the saved UI history instead of attaching to an unrelated “last” CLI session. Bare `codex`, planning, workers, synthesis, and compaction remain stateless. Codex planning/workers use isolated one-turn app-server processes; Codex synthesis and compaction use fresh `codex exec` processes. Images are represented in CLI handoffs but their binary contents aren't forwarded to these providers yet (they work with the API providers).
